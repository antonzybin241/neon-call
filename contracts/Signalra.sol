...
/**
 * @title Signalra
 * @notice Platform credit system for the Signalra token voting & promotion dApp.
 *         Users purchase Signalra credits with ETH. Credits can be spent to vote
 ...
 */
contract Signalra is ReentrancyGuard, Ownable, Pausable {

    // ─── State ────────────────────────────────────────────────────────────

    /// @notice Signalra credit balance per address
    ...
    constructor(address payable _treasury) Ownable(msg.sender) {
        require(_treasury != address(0), "Signalra: zero treasury");
        treasury = _treasury;
    }
    ...
    /**
     * @notice Purchase Signalra credits by sending one of the accepted ETH amounts.
    ...
    function buyCredits() external payable nonReentrant whenNotPaused {
        ...
        require(reward > 0, "Signalra: invalid ETH amount");
    ...
    function buyRubyTier() external payable nonReentrant whenNotPaused {
        require(msg.value == rubyTierPrice, "Signalra: incorrect ruby price");
    ...
    function buyDiamondTier() external payable nonReentrant whenNotPaused {
        require(msg.value == diamondTierPrice, "Signalra: incorrect diamond price");
    ...
    function spendCredits(uint256 amount, string calldata reason) external nonReentrant {
        require(credits[msg.sender] >= amount, "Signalra: insufficient credits");
    ...
    function setTreasury(address payable _treasury) external onlyOwner {
        require(_treasury != address(0), "Signalra: zero address");
    ...
    function setCreditRate(uint256 index, uint256 ethAmount, uint256 creditsGiven) external onlyOwner {
        require(index < 9, "Signalra: index out of range");
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        require(balance > 0, "Signalra: nothing to withdraw");
        (bool ok, ) = treasury.call{value: balance}("");
        require(ok, "Signalra: transfer failed");
